...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "madhav-ascii.svg")
...
SHARPEN = False
WHITE_FLOOR = 0.94    # luminance above this is forced to blank (space)
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">madhav@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'madhav@github:~$ whoami <tspan fill="{INK}">Madhav N Joshi</tspan></text>')
