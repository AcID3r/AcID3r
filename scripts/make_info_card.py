...
ROWS = [
    ("host",),
    ("kv", "Name", "Madhav N Joshi"),
    ("kv", "Focus", "AI/ML, Robotics, Quantum"),
    ("kv", "Tagline", "Building smarter technology"),
    ("kv", "Edu", "Advanced DS/ML - IIT Guwahati"),
    ("kv", "Intern", "IIT Kanpur / Upflairs"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "AI / ML", "PyTorch, TensorFlow, Keras"),
    ("kv", "LLM", "LangChain, LangGraph, RAG"),
    ("kv", "Vision", "OpenCV, MediaPipe"),
    ("kv", "Quantum", "Qiskit"),
    ("gap",),
    ("sec", "Featured Systems"),
    ("bul", "Live stock monitor + Discord alerts"),
    ("bul", "Gesture gaming with OpenCV"),
    ("bul", "CNN healthcare diagnostics"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">madhav@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">madhav</tspan><tspan fill="{MUTED}">@</tspan>'
